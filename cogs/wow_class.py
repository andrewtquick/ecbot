...
class WowClass(commands.Cog):
    ...
    async def demon_hunter(self, ctx: Context, *, spec=None):
        ...
        if spec == None:
            await ctx.send(f'{ctx.author.mention} -> Which spec? <:havoc:797204927897665568> Havoc or <:vengeance:797204927939739658> Vengeance?')
            msg = await self.bot.wait_for('message', check=lambda message: message.author == ctx.author, timeout=5)
    ...
    async def druid(self, ctx: Context, *, spec=None):
        ...
        if spec == None:
            await ctx.send(f'{ctx.author.mention} -> Which spec? <:feral:797202712156307507> Feral, <:guardian:797202712369561601> Guardian, <:restoration:797202712382537767> Resto, or <:balance:797202712156307566> Balance?')
            msg = await self.bot.wait_for('message', check=lambda message: message.author == ctx.author, timeout=5)
    ...
    async def mage(self, ctx: Context, *, spec=None):
        ...
        if spec == None:
            await ctx.send(f'{ctx.author.mention} -> Which spec? <:mfire:797202712209915975> Fire, <:mfrost:797202712448991262> Frost, or <:arcane:790339486600921158> Arcane?')
            msg = await self.bot.wait_for('message', check=lambda message: message.author == ctx.author, timeout=5)

            if msg.content.lower() == 'fire':
                await ctx.send(f'{ctx.author.mention} -> Here is the Fire Guide on WowHead\nhttps://www.wowhead.com/fire-mage-guide')
            if msg.content.lower() == 'frost':
                await ctx.send(f'{ctx.author.mention} -> Here is the Frost Guide on WowHead\nhttps://www.wowhead.com/frost-mage-guide')
            if msg.content.lower() == 'arcane':
                await ctx.send(f'{ctx.author.mention} -> Here is the Arcane Guide on WowHead\nhttps://www.wowhead.com/arcane-mage-guide')
    ...
    async def priest(self, ctx: Context, *, spec=None):
        ...
        if spec == None:
            await ctx.send(f'{ctx.author.mention} -> Which spec? <:discipline:797202712004526172> Discipline, <:shadow:797202712374280243> Shadow, or <:holy:797202712479006810> Holy?')
            msg = await self.bot.wait_for('message', check=lambda message: message.author == ctx.author, timeout=5)

            if msg.content.lower() == 'discipline' or msg.content.lower() == 'disc':
                await ctx.send(f'{ctx.author.mention} -> Here is the Discipline Guide on WowHead\nhttps://www.wowhead.com/discipline-priest-guide')
            ...
            if msg.content.lower() == 'holy':
                await ctx.send(f'{ctx.author.mention} -> Here is the Holy Guide on WowHead\nhttps://www.wowhead.com/holy-priest-guide')
    ...
    async def rogue(self, ctx: Context, *, spec=None):
        ...
        if spec == None:
            await ctx.send(f'{ctx.author.mention} -> Which spec? <:assassination:797202712290787399> Assassination, <:subtlety:797206736041803796> Subtlety, or <:outlaw:797202712310841354> Outlaw?')
            msg = await self.bot.wait_for('message', check=lambda message: message.author == ctx.author, timeout=5)
    ...
    async def shaman(self, ctx: Context, *, spec=None):
        ...
        if spec == None:
            await ctx.send(f'{ctx.author.mention} -> Which spec? <:enhancement:797202712160501770> Enhancement, <:elemental:797202712247795772> Elemental, or <:srestoration:797202712382668800> Resto?')
            msg = await self.bot.wait_for('message', check=lambda message: message.author == ctx.author, timeout=5)

            if msg.content.lower() == 'enhancement' or msg.content.lower() == 'enh':
                await ctx.send(f'{ctx.author.mention} -> Here is the Enhancement Guide on WowHead\nhttps://www.wowhead.com/enhancement-shaman-guide')
            ...
            if msg.content.lower() == 'resto':
                await ctx.send(f'{ctx.author.mention} -> Here is the Restoration Guide on WowHead\nhttps://www.wowhead.com/restoration-shaman-guide')
    ...
    async def warrior(self, ctx: Context, *, spec=None):
        ...
        if spec == None:
            await ctx.send(f'{ctx.author.mention} -> Which spec? <:arms:797202712201527316> Arms, <:fury:797202712264704000>  Fury, or <:wprotection:797202712718082068> Protection?')
            msg = await self.bot.wait_for('message', check=lambda message: message.author == ctx.author, timeout=5)

            if msg.content.lower() == 'arms':
                await ctx.send(f'{ctx.author.mention} -> Here is the Arms Guide on WowHead\nhttps://www.wowhead.com/arms-warrior-guide')
            if msg.content.lower() == 'fury':
                await ctx.send(f'{ctx.author.mention} -> Here is the Fury Guide on WowHead\nhttps://www.wowhead.com/fury-warrior-guide')
            if msg.content.lower() == 'protection' or msg.content.lower() == 'prot':
                await ctx.send(f'{ctx.author.mention} -> Here is the Protection Guide on WowHead\nhttps://www.wowhead.com/protection-warrior-guide')
