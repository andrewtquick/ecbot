...
class Gamble(commands.Cog):
    ...
    @cog_ext.cog_slash(
        ...
        options=[
            create_option(
                name='amount',
                description='How much do you want to gamble for?',
                required=True,
                option_type=3
            ),
            create_option(
                name='autoroll',
                description='Should the bot auto roll for everyone?',
                required=True,
                option_type=3,
                choices=[
                    create_choice(
    ...
    async def start_game(self, ctx: Context, amount: int, autoroll='on'):
        ...
        if ctx.channel == gamble_chan:
            if self.active_game == False:
                ...
                embed.set_thumbnail(url='https://i.imgur.com/ATlU9uu.png')
                ...
                await ctx.send(content=f'{ctx.author.mention} -> There is already an active game. Use command .current for more information', delete_after=10)
        else:
            await ctx.send(content=f'{ctx.author.mention} -> Sorry, gambling is only available in the #gambling channel.', delete_after=10)
    ...
    async def on_raw_reaction_add(self, payload):
        ...
        if payload.channel_id == int(self.GAMBLE_CHAN):
            ...
            if payload.emoji.name == '🗑️' and payload.member.name == self.game_creator:
                ...
                cancel_embed.set_thumbnail(url='https://i.imgur.com/ATlU9uu.png')
                ...
                roll_embed.set_thumbnail(url='https://i.imgur.com/ATlU9uu.png')
                ...
                if payload.emoji.name == '🎲' and len(self.gamblers) <= 1 and payload.member.name == self.game_creator:
                    await chan.send(f'{payload.member.name} -> We need more people to join before starting a game.')

            if payload.emoji.name == '🎲' and payload.member.name != self.game_creator:
                await chan.send(f"{payload.member.name} -> Sorry, only **{self.game_creator}** can start the game.", delete_after=10)
            if payload.emoji.name == '🗑️' and payload.member.name != self.game_creator:
                await chan.send(f"{payload.member.name} -> Sorry, only **{self.game_creator}** can cancel the game.", delete_after=10)
        else:
            return
    ...
    async def begin_roll(self):
        ...
        if self.auto_roll == False:
            roll_embed = discord.Embed(title="Time to roll!", description=f"Type **/gambleroll** now!", colour=discord.Color.red())
            roll_embed.set_thumbnail(url='https://i.imgur.com/ATlU9uu.png')
            roll_embed.add_field(name='Current Gamblers', value='\n'.join(g for g in self.gamblers), inline=False)
        ...
        if ctx.channel.id == int(self.GAMBLE_CHAN):
            if ctx.author.name in self.gamblers:
                if ctx.author.name in self.gamblers_rolled:
                    await ctx.send(content=f"{ctx.author.mention} -> Sorry, you've already rolled for this session.", delete_after=10)
                else:
                    random_num = Random(1, int(self.gamble_amount))
                    self.gamblers_rolled[ctx.author.mention] = random_num
                    await ctx.send(content=f'{ctx.author.mention} -> You rolled a {random_num}')
                    ...
                    if len(self.gamblers_rolled) == len(self.gamblers):
                        ...
                        for k,v in self.gamblers_rolled.items():
                            ...
                            if v == lowest_roll:
                                loser = k
                        await ctx.send(content=f'{loser} must pay **{payout}<:goldcoin:844724052294893568>** to {winner}!')
                        self.gamblers = []
                        self.gamblers_rolled = {}
                        self.gamble_amount = 0

            else:
                await ctx.send(content=f"{ctx.author.mention} -> Sorry, you didn't enter before the game closed. Please join the next game.", delete_after=10)
        else:
            return
    ...
    async def list_gamblers(self, ctx: Context):
        ...
        if ctx.channel.id == self.gamble_chan:
            if len(self.gamblers) == 0:
                await ctx.send(content=f"{ctx.author.mention} -> There is not an active game currently. To start a game, type **.gamble <amount>**", delete_after=10)
            else:
                ...
                gambler_embed.set_thumbnail(url='https://i.imgur.com/ATlU9uu.png')
                ...
                await ctx.send(content='', embed=gambler_embed)
        else:
            await ctx.send(content=f'{ctx.author.mention} -> Sorry, gambling is only available in the #gambling channel.', delete_after=10)
    ...
    async def current_game(self, ctx: Context):
        ...
        if ctx.channel.id == int(self.GAMBLE_CHAN):
            if self.active_game == False:
                await ctx.send(content=f"{ctx.author.mention} -> Sorry, there isn't a current game. To start a game type /gamble")
            else:
                await ctx.send(content=f"{ctx.author.mention} -> There is a current game, scroll up to join.")
        else:
            return
